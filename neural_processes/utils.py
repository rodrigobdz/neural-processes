...
def preprocess_mnist(Y, train=True):
    ...
    target_idx = torch.stack(
        [torch.randperm(28*28)[:num_total_points] for _ in range(batch_size)])
    # map linear indices to Cartesian coordinates
    target_row, target_col = unravel(target_idx)
...
def reconstruct(np_img, num_img, num_samples, test_generator, num_context=[10, 100, 300, 784], random_order=True):
    ...
    for i, val in enumerate(test_generator):
        ...
        for context in num_context:
            context_x, context_y, target_x, _ = sample_context_target(
                img, context, random_order)
...
def sample_context_target(img, num_context, random_order):
    ...
    if random_order:
        ...
        target_idx = torch.stack([torch.arange(28*28)
    ...
    # [B, num_total_point, 1]
    target_y = torch.cat([y[i.tolist()] for y, i in zip(Y, target_idx)]).reshape(
        batch_size, num_total_points, 1)  # TODO
